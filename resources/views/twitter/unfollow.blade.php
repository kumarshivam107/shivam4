...
@section('content')
    ...
    <section class="content">
        <div class="callout callout-info">
            <h4>{{__('Set up Unfollow Schedule!')}}</h4>
            <p>{{__('This feature allows you to setup auto unfollow of non-followers on your twitter account.')}}</p>
            <p>{{__('Note: There is a delay between every scheduled activities. This is necessary in order to comply with Twitter\'s terms and conditions.')}}</p>
            <p class="alert alert-warning">
                <strong>{{__('Important!')}}</strong> {{__('Only 25 profiles are Unfollowed per 5 minutes, and also only one schedule per account can be active at a time.')}}
            </p>
        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group form-md-line-input">
                                            <label for="account">{{__('Select a Twitter Account')}}</label>
                                            <select id="account" class="form-control select2" name="twitter_account">
                                                <option></option>
                                                @foreach($tw_accounts as $tw_account)
                                                    <option value="{{$tw_account->id}}">{{$tw_account->profile_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        var AutoCompleteInit = function() {
            $('.js-data-ajax').select2({
                ...
                ajax: {
                    url: '{{route('search-tw-non-followers')}}',
                    dataType: 'json',
                    type: 'POST',
                    delay: 250,
